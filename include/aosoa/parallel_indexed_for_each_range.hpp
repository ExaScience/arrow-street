  ...
  namespace {
	...
	public:
	  ...
	  static inline void cilk_loop(C& container, const F& f) {
		...
		if (smb) cilk_spawn f(data[sdb], 0, smb, sdb*traits::table_size);
		cilk_for (size_t i=0; i<sdb; ++i) f(data[i], 0, traits::table_size, i*traits::table_size);
	  }
#endif
	...
	public:
	  ...
	  static inline void cilk_loop(C& container, const F& f) {
		...
		cilk_for (auto it=begin; it<end; it+=grainsize) f(it, 0, std::min(grainsize, end-it), it-begin);
	  }
#endif
	};
  }
...
#ifdef __cilk
  template<class C, typename F>
  inline void cilk_parallel_indexed_for_each_range(C& container, const F& f)
  {_parallel_indexed_for_each_range<C>::cilk_loop(container, f);}
#endif
  ...
  namespace {
	...
	public:
	  ...
	  static inline void cilk_loop(T begin, T end, const F& f) {
		...
		if (table0 < tablen) {
		  cilk_spawn f(table0[0], index0, traits::table_size, -index0);
		  const auto range = tablen-table0;
		  cilk_spawn f(tablen[0], 0, indexn, range*traits::table_size-index0);
		  cilk_for (ptrdiff_t i=1; i<range; ++i)
			f(table0[i], 0, traits::table_size, i*traits::table_size-index0);
		} else if (table0 == tablen) {
...
#ifdef __cilk
  template<typename T, typename F>
  inline void cilk_parallel_indexed_for_each_range(T begin, T end, const F& f)
  {_parallel_indexed_for_each_range_it<T>::cilk_loop(begin, end, f);}
#endif
}
